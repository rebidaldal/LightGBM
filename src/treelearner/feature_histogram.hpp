...
public:
  ...
  */
  void Init(double* data, const FeatureMetainfo* meta) {
    meta_ = meta;
    data_ = data;
  ...
  */
  ...
  void FindBestThresholdCategorical(double sum_gradient, double sum_hessian, data_size_t num_data,
    ...
    if (use_onehot) {
      for (int t = 0; t < used_bin; ++t) {
        const auto grad = GetGrad(data_, t);
        const auto hess = GetHess(data_, t);
      ...
      for (int i = 0; i < used_bin; ++i) {
        if (GetHess(data_, i) * cnt_factor >= meta_->config->cat_smooth) {
      ...
      std::sort(sorted_idx.begin(), sorted_idx.end(),
        [this, &ctr_fun](int i, int j) {
          return ctr_fun(GetGrad(data_, i), GetHess(data_, i)) < ctr_fun(GetGrad(data_, j), GetHess(data_, j));
        });
  ...
  void GatherInfoForThresholdCategorical(double sum_gradient, double sum_hessian,
    ...
    const double cnt_factor = num_data / sum_hessian;
    const auto grad = GetGrad(data_, threshold);
    const auto hess = GetHess(data_, threshold);
    data_size_t cnt = static_cast<data_size_t>(hess * cnt_factor);
  ...
  */
  int SizeOfHistgram() const {
    return (meta_->num_bin - meta_->offset) * sizeof(double) * 2;
  }
  ...
  */
  void FromMemory(char* memory_data) {
    std::memcpy(data_, memory_data, (meta_->num_bin - meta_->offset) * sizeof(double) * 2);
  }
    ...
    if (dir == -1) {
      ...
      if (use_na_as_missing && offset == 1) {
        ...
        for (int i = 0; i < meta_->num_bin - offset; ++i) {
          const auto grad = GetGrad(data_, i);
          const auto hess = GetHess(data_, i);
          data_size_t cnt = static_cast<data_size_t>(hess * cnt_factor);
          sum_left_gradient -= grad;
          sum_left_hessian -= hess;
          left_count -= cnt;
      ...
      for (; t <= t_end; ++t) {
        ...
        if (skip_default_bin && (t + offset) == static_cast<int>(meta_->default_bin)) { continue; }
        if (t >= 0) {
          sum_left_gradient += GetGrad(data_, t);
          sum_left_hessian += GetHess(data_, t);
          left_count += static_cast<data_size_t>(GetHess(data_, t) * cnt_factor);
        }
        // if data not enough, or sum hessian too small
        if (left_count < meta_->config->min_data_in_leaf
          || sum_left_hessian < meta_->config->min_sum_hessian_in_leaf) continue;
